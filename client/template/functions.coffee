...
Template.look_in_options.option = ->
  ensure.inside "Template.look_in_options.option"
  selected = App.session "data.look_in_selected"
  seq = ["center", "batch", "group"]
  selector = {}
  for v, i in selected
    selector[seq[i]] = selected[i].doc_id
  App.find.map seq[selected.length], selector, (doc, i)->
